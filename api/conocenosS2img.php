<?php 
    ...
    if(!empty($_FILES['nfileImg']))
    {
      ...
      if(move_uploaded_file($_FILES['nfileImg']['tmp_name'], $path)) {
        ...
        $datos = "'$newPath'";
        $mewID =  $idimg + 1;
        $r = $user->actualizar("UPDATE c_secciondosimg set img2 = $datos WHERE id = $idimg");
        $r = $user->actualizar("UPDATE c_secciondosimg set img2 = $datos WHERE id = $mewID");
        ...
        if($r == true){
            ...
            echo "<script>location='../dashboard/conocenossecciondos.php'</script>";
        }
      } else{
