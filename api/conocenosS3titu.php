<?php 
    ...
    if(!empty($_FILES['nfileImg']))
    {
      ...
      if(move_uploaded_file($_FILES['nfileImg']['tmp_name'], $path)) {
        ...
        $datos = "'$newPath'";
        //$mewID =  $idimg + 1;
        $r = $user->actualizar("UPDATE c_secciontres set img = $datos WHERE id = $idimg");
        ...
        if($r == true){
            ...
            //echo $newPathUnlik;
            //header("Location: ../dashboard/conocenossecciontres.php");
            echo "<script>location='../dashboard/conocenossecciontres.php'</script>";
        }
      } else{
