<?php 
    ...
    if(!empty($_FILES['nfileImg']))
    {
      ...
      if(move_uploaded_file($_FILES['nfileImg']['tmp_name'], $path)) {
        ...
        $datos = "'$newPath'";
        $mewID =  $idimg + 1;
        $r = $user->actualizar("UPDATE c_seccionuno set img2 = $datos WHERE id = $idimg");
        ...
        if($r == true){
            ...
            //echo $newPathUnlik;
            //header("Location: ../dashboard/conocenosseccionuno.php");
            echo "<script>location='../dashboard/conocenosseccionuno.php'</script>";
        }
      } else{
